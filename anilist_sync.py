...
def normalize_mal_cover_url(url):
    if not url:
        return ""
...
def fetch_mal_user(username):
    ...
    while True:
        ...
        for idx, entry in enumerate(items):
            ...
            cover = normalize_mal_cover_url(
                entry.get("anime_image_path")
                or entry.get("anime_image_url")
...
def get_cover_for_mid(all_data, mid):
    for anime_map in all_data.values():
        if mid in anime_map:
            return anime_map[mid].get("cover", "") or ""
    return ""


def get_source_for_mid(all_data, mid):
    for anime_map in all_data.values():
        if mid in anime_map:
            return anime_map[mid].get("source", "") or ""
    return ""
...
def write_site_data_json(master, grid, analytics, all_data, last_updated):
    ...
    Estrutura esperada pelo docs/index.html:
    {
      "updated": "27/06/2026  10:30",
      "users": [...],
      "animes": [
        {
          "id": "123",
          "title": "Nome",
          "cover": "https://...",
          "statuses": {"Usuario": "Assistido"}
        }
      ]
    }
    """

    watched_count = analytics.get("watched_count", {})

    sorted_ids = sorted(
        master,
        key=lambda mid: (
            -watched_count.get(mid, 0),
            master[mid].lower(),
        ),
    )
    ...
    payload = {
        ...
        "animes": animes,
        "summary": {
            "total_unique": analytics.get("total_unique", len(master)),
            "all_watched_by_all": analytics.get("all_watched_by_all", 0),
        },
    }
    ...
    print(f"data.json gerado: {SITE_DATA_PATH} ({len(animes)} animes)")
...
def run_sync(spreadsheet, verbose=True):
    ...
    write_site_data_json(master, grid, analytics, all_data, last_updated)
