...
def main():
    ...
    agora = now_br("%H:%M:%S")
    print(f"[{agora}] Buscando listas (com capas para o site)...")

    # Busca AniList com capas (para o site) + MAL normalmente
    all_data = {}
    for u in ANILIST_USERNAMES:
        print(f"  [AniList] -> {u}...", end=" ", flush=True)
        all_data[u] = fetch_user_anime_list_with_covers(u)
        print(f"{len(all_data[u])} animes")
        time.sleep(1)
    for u in MAL_USERNAMES:
        print(f"  [MAL]     -> {u}...", end=" ", flush=True)
    ...
    # Gera data.json para o GitHub Pages
    print("\nGerando docs/data.json para o site...")
    json_data = build_json_data(all_data, master, grid)
    write_json_file(json_data)

    agora2 = now_br("%H:%M:%S")
    print(f"[{agora2}] Tudo atualizado com sucesso!")
...
# ─────────────────────────────────────────────
# GERAÇÃO DO data.json PARA O GITHUB PAGES
# ─────────────────────────────────────────────

MEDIA_LIST_WITH_COVER_QUERY = """
query ($username: String) {
  MediaListCollection(userName: $username, type: ANIME) {
    lists {
      entries {
...
# Cache de capas buscadas (evita re-buscar por anime já encontrado)
_cover_cache = {}

def fetch_user_anime_list_with_covers(username):
    """Versão da busca AniList que também retorna URL da capa."""
    r = requests.post(
        ANILIST_API_URL,
        json={"query": MEDIA_LIST_WITH_COVER_QUERY, "variables": {"username": username}},
        headers={"Content-Type": "application/json", "Accept": "application/json"},
        timeout=30,
    )
    if r.status_code == 429:
        ...
        print(f"  Rate limit AniList! Aguardando {wait}s...")
        time.sleep(wait)
        return fetch_user_anime_list_with_covers(username)
    r.raise_for_status()
    data = r.json()
    if "errors" in data:
        return {}
...
def build_json_data(all_data_with_covers, master, grid):
    """
    Monta o dict que será salvo em docs/data.json.
    Cada anime tem: id, title, cover, statuses (dict user→status).
    """
    animes = []
    for mid, title in sorted(master.items(), key=lambda x: x[1].lower()):
        cover = _cover_cache.get(mid, "")
        statuses = {u: grid[mid].get(u, "-") for u in USERNAMES}
        animes.append({
            "id":       str(mid),
            "title":    title,
            "cover":    cover,
...
def write_json_file(data_dict):
    """Salva docs/data.json na raiz do repositório (onde o Actions roda)."""
    os.makedirs("docs", exist_ok=True)
    path = os.path.join("docs", "data.json")
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data_dict, f, ensure_ascii=False, separators=(",", ":"))
    print(f"  docs/data.json gerado ({len(data_dict['animes'])} animes)")
