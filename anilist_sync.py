...
def normalize_mal_status(raw_status):
    """
    Normaliza os status retornados pelo MyAnimeList/Jikan.

    A Jikan pode retornar status como:
    - "Completed"
    - "Watching"
    - "On-Hold"
    - "Dropped"
    ...
    Também pode retornar versões minúsculas dependendo do endpoint/cache.
    """
...
def fetch_mal_user(username):
    ...
    - A lista do MAL precisa estar pública.
    - A Jikan não acessa listas privadas.
    - Usa mal_id com prefixo 'mal_' para evitar colisão com IDs do AniList.
    """
...
def compute_hash(all_data):
    snap = {
        u: sorted(
            [(mid, info["status"]) for mid, info in am.items()],
...
def build_analytics(master, grid):
    ...
    all_watched_by_all = sum(
        1 for c in watched_count.values()
        if c == N
    )

    user_stats = {}

    status_values = list(STATUS_MAP_ANILIST.values())
    ...
    most_watched_user = max(
        USERNAMES,
        key=lambda u: user_stats[u]["Assistido"],
    )

    most_dropped_user = max(
        USERNAMES,
        key=lambda u: user_stats[u]["Dropado"],
    )

    biggest_list_user = max(
        USERNAMES,
...
def write_sync_sheet(ws, master, grid, analytics, last_updated):
    ...
    reqs.append(
        border_req(
            ...
            0,
            total_cols,
            color={"red": 0.58, "green": 0.62, "blue": 0.70},
            inner=True,
        )
    )
...
def write_stats_sheet(ws, master, grid, analytics):
    ...
    reqs.append(
        repeat_cell(
            ...
            3,
            cell_fmt(
                STATUS_BG["Assistindo"],
                STATUS_FG["Assistindo"],
                bold=True,
                size=10,
                align="CENTER",
            ),
    ...
    reqs.append(
        repeat_cell(
            ...
            4,
            cell_fmt(
                STATUS_BG["Planejando"],
                STATUS_FG["Planejando"],
                bold=True,
                size=10,
                align="CENTER",
            ),
...
def test_mal():
    ...
    for user in MAL_USERNAMES:
        ...
        primeiros = list(data.items())[:10]

        for mid, info in primeiros:
            print(mid, "=>", info)
...
def main():
    ...
    print(
        f"  AniList: {len(ANILIST_USERNAMES)} usuários | "
        f"MAL: {len(MAL_USERNAMES)} usuário(s)"
    )
    print("=" * 55)
...
if __name__ == "__main__":
    ...
    # Para testar só o MAL, comente a linha acima e descomente esta:
    # test_mal()
